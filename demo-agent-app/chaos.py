...
Currently implements only the Stage 3 rules, per the build spec's own
...
R3/R6/R7 triggers are NOT implemented here yet -- Section 6 Stage 4 is
where this file gets extended for those (R7's trigger also needs
citation_service.py to exist first, Stage 7). No stub functions for those
are added now, same discipline the spec applies to R5.

Usage:
    CHAOS_MODE=1 python app.py --question "..." --pdf fixtures/long_climate_report.pdf

Env vars:
    CHAOS_MODE       "1"/"true" to enable. Default off -- untouched runs
                     behave exactly as Stage 1/2 already verified.
...
def install_chaos() -> None:
    ...
    global _installed, _original_set_attribute, _rng, _r1_rate, _r2_rate
    ...
    print(f"[chaos] installed -- R1 rate={_r1_rate}, R2 rate={_r2_rate}, seed={seed or 'random'}")
